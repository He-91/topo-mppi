...
## 项目简介
基于Fast-Planner思想的三层无人机路径规划系统：
- **TopoPRM**（全局多拓扑路径，PRM v3.0）
- **MPPI**（动力学轨迹并行优化）
- **B-spline**（轨迹平滑）
...
### TopoPRM PRM v3.0核心算法
1. 椭球自由空间采样（100点，clearance=0.8m）
2. 全局可见性图构建（102节点，10m半径）
3. DFS多路径搜索（最多50条原始路径，深度20）
4. 拓扑等价性去重（30点离散化，5-13条唯一路径）
5. 最优路径选择（保留8条最短，输出1-4条）
...
| 参数 | 当前 | 建议 | 作用 |
|------|------|------|------|
| sample_inflate_ | 3.0 | 4.0 | 扩大采样区域 |
| ratio_to_short_ | 2.5 | 3.5 | 保留更多路径 |
| discretize_points_num_ | 30 | 20 | 放宽去重 |
| max_raw_paths_ | 50 | 100 | 增加DFS深度 |

---

## MPPI可视化说明
- RViz可实时显示采样轨迹、最优轨迹、速度箭头
- 话题：`/mppi_trajectories`（采样轨迹，最多50条，权重着色）
- 话题：`/mppi_optimal_trajectory`（最优轨迹+速度箭头）
- 采样轨迹线宽0.05m，最优轨迹0.15m，速度箭头蓝色
...
## 版本历史与升级路线
- v1.0: 单路径B-spline（原始EGO-Planner）
- v2.0: 障碍物切线点法（多路径率18.75%）
- v3.0: Fast-Planner PRM（多路径率58.06%，并行MPPI）
  - Week 1: 椭球采样
  - Week 2: 可见性图
  - Week 3: DFS多路径
  - Week 4: 拓扑去重
...
**当前状态**: ⚠️ 已将 MPPI 改为在每次规划周期中强制运行（若 `mppi_planner_` 可用），以保证动态可行性和一致优化结果。
...
| **生成路径数量** | 1条 | 2-3条 |
| **Parallel MPPI运行** | **已调整为每次触发（若可用）** | 每次规划 |
